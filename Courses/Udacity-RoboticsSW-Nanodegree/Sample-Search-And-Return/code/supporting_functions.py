...
def update_rover(Rover, data):
      ...
      if Rover.start_time == None:
            ...
            Rover.blocked_start_time = time.time()
      # Or just update elapsed time
      else:
            tot_time = time.time() - Rover.start_time
            if np.isfinite(tot_time):
                  Rover.total_time = tot_time
            if Rover.vel < 0.05 and convert_to_float(data["speed"]) < 0.05:
                  if (time.time() - Rover.blocked_start_time) > 20:
                        Rover.mode = 'blocked'
            elif convert_to_float(data["speed"]) < 0.05:
                  Rover.blocked_start_time = time.time()
            else:
                  pass
      ...
      print('mode = ', Rover.mode ,'speed =',Rover.vel, 'throttle =',
      Rover.throttle, 'steer_angle =', Rover.steer, 'samples remaining:', data["sample_count"],
